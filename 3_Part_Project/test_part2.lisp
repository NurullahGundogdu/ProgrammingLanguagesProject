...
(defun merge-list (list1 list2)
	(cond ((null list1) list2)     ;;if list1 is empty return list2 
		(t (cons (first list1)			;;the first element of the first list is taken
			(merge-list (rest list1) list2))		;;added at the end of the second list
			)
	)
)
...
(defun main ()
  (with-open-file (stream #p"input_part2.csv")
    (loop :for line := (read-csv-line stream) :while line :collect
      (format t "~a~%" (merge-list (read-from-string (nth 0 line)) (read-from-string (nth 1 line)))
